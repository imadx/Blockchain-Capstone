...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(msg.sender == _owner, "Can be called only by the owner");
        _;
    }
...
contract Pausable is Ownable {
    ...
    modifier whenNotPaused() {
        require(_paused == false, "Requires not to be paused");
        _;
    }

    modifier paused() {
        require(_paused == true, "Requires the contract to be paused");
        _;
    }
}
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        require(
            to != ownerOf(tokenId),
            "Requires to address not to be the owner of tokenId"
        );

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(
            owner() == msg.sender || isApprovedForAll(owner(), to),
            "Requires msg.sender to be owner or isApprovedForAll"
        );
    ...
    function _transferFrom(
        ...
        require(
            ...
            "Requires from address to be the owner of tokenId"
        );

        // TODO: require token is being transfered to valid address
        require(to != address(0), "Requires to address to be a valid address");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function setTokenURI(uint256 tokenId) public {
        require(_exists(tokenId), "Requires tokenId to exist");
